...
int main(void) {
    ...
    if (!check_va(vaCreateContext(display, config, smoke_width, smoke_height, VA_PROGRESSIVE,
                                  &surfaces[0], 1, &contexts[0]),
                  "vaCreateContext[0]") ||
        !check_va(vaCreateContext(display, config, smoke_width, smoke_height, VA_PROGRESSIVE,
                                  &surfaces[1], 1, &contexts[1]),
                  "vaCreateContext[1]")) {
        cleanup();
        return 1;
    }
    ...
    if (!check_va(vaQuerySurfaceStatus(display, surfaces[1], &status), "vaQuerySurfaceStatus[1]") ||
