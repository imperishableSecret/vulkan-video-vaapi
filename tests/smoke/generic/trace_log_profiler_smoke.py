...
nvidia-vulkan-vaapi: trace seq=59 event=seed-target-pixel-proof target_surface=7 source_surface=6 target_fd_dev=11 target_fd_ino=22 target_crc_after_copy=0xabc source_crc=0xabc matches_source=1 target_is_black=0 target_is_zero=0 pixel_proof_valid=1 pixel_proof_state=valid-nonblack reject_reason=none copy_status=success source_sample_bytes=8192 target_sample_bytes=8192 sample_bytes=8192
...
def main() -> int:
    ...
    totals = data["totals"]
    check(data["trace_records"] == 75, "trace record count mismatch")
    ...
    check("export_role=predecode-target" in FIXTURE and "decision=fail reason=predecode-target-needs-proven-seed returned_fd=0" in FIXTURE,
          "predecode target failure fixture missing")
    check("event=predecode-target-export-refused" in FIXTURE and "status=20" in FIXTURE, "predecode target nonfatal status fixture missing")
    check(data["events"].get("predecode-target-export-refused") == 1, "predecode target refusal telemetry count mismatch")
    check(
