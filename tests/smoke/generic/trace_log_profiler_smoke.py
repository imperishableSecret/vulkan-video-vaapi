...
nvidia-vulkan-vaapi: trace seq=16 event=observable-fd-decode-refresh surface=7 driver=2 stream=1 codec=0x8 refresh_export=0 display_visible=0 content_gen=2 fd_content_gen_before=1 fd_content_gen_after=2 present_gen=0 display_published=0 presentable=0 present_pinned=0 presentation_separate=1
...
nvidia-vulkan-vaapi: trace seq=56 event=predecode-target-seed-state surface=7 target_surface=7 source_surface=6 driver=2 stream=1 codec=0x8 source_present_gen=1 source_fd_content_gen=1 source_fd_matches_present=1 target_content_gen_before=0 target_content_gen=0 target_content_preserved=1 target_fd_content_gen_after=1 target_last_written_content_gen_after=1 target_pixel_source=seed seed_generation_basis=source-present target_presentable=0 target_published_visible=0 target_predecode_quarantined=1 target_predecode_exported=1
...
nvidia-vulkan-vaapi: trace seq=58 event=export-present-state action=predecode-target-seeded surface=7 codec=0x8 stream=1 fd_dev=11 fd_ino=22 content_gen=0 present_shadow_gen=0 private_shadow_gen=0 decode_shadow_gen=0 fd_exported=0 fd_content_gen=1 may_be_sampled_by_client=0 fd_observable=0 detached_from_surface=0 present_gen=0 presentable=0 present_pinned=0 published_visible=0 decode_shadow_private_active=0 predecode=1 seeded=1 predecode_quarantined=1 predecode_generation=0 placeholder=0 export_role=predecode-target export_intent=read-only raw_export_flags=0x5 mem_type=0x40000000 had_va_begin=0 had_decode_submit=0 refresh_export=1 display_visible=0 present_source=predecode-placeholder mutation_action=none client_visible_shadow_mutated=0 client_visible_shadow=0 private_only=0 external_release_required=0 external_release_done=0 external_release_mode=none-required external_released_generation=0
nvidia-vulkan-vaapi: trace seq=59 event=fd-observability surface=7 driver=2 stream=1 codec=0x8 fd_dev=11 fd_ino=22 export_intent=read-only fd_exported=1 fd_observable=1 readable_intent=1 detached_from_surface=0 may_be_sampled_by_client=1 reason=mark-returned
nvidia-vulkan-vaapi: trace seq=59 event=predecode-target-export-return surface=7 driver=2 fd_dev=11 fd_ino=22 stream=1 codec=0x8 content_gen=0 fd_content_gen=1 presentable=0 published_visible=0 predecode_quarantined=1 may_be_sampled_by_client=1 fd_observable=1 seed_used=1 seed_source_surface=6 seed_source_generation=1 export_role=predecode-target export_intent=read-only status=0
...
nvidia-vulkan-vaapi: trace seq=61 event=pixel-proof-computed surface=7 fd_dev=11 fd_ino=22 generation=1 pixel_source=decoded crc=0xabc black_crc=0x10 zero_crc=0x0 is_black=0 is_zero=0 sample_bytes=8192 state=valid-nonblack pixel_proof_mode=seed
nvidia-vulkan-vaapi: trace seq=62 event=generic-export-summary surface=7 stream=1 codec=0x8 width=3840 height=2160 fourcc=0x30313050 content_gen=1 fd_content_gen=1 returned_fd=1 decision=return-decoded pixel_source=decoded pixel_proof_valid=1 is_black=0 is_zero=0 pending_decode=0 valid_seed_available=0 quarantine_outcome=none external_release_mode=implicit-sync-only status=0 va_status=0 actual_mem_type_supported=1 policy_status=success may_be_sampled_by_client=1 export_role=decoded-presentation export_intent=read-only raw_export_flags=0x5 mem_type=0x40000000 had_va_begin=0 had_decode_submit=0
...
def main() -> int:
    ...
    check("decode_crc=0x10 black_crc=0x10 zero_crc=0x0 is_black=1 is_zero=0 pixel_proof_valid=1" in FIXTURE, "black decoded pixel proof fixture missing")
    ...
    check("event=predecode-target-seed-state" in FIXTURE and "target_fd_content_gen_after=1" in FIXTURE and "target_pixel_source=seed" in FIXTURE
          and "source_fd_matches_present=1" in FIXTURE and "target_content_preserved=1" in FIXTURE and "seed_generation_basis=source-present" in FIXTURE,
    ...
    check("event=predecode-target-export-return" in FIXTURE and "seed_used=1" in FIXTURE and "may_be_sampled_by_client=1 fd_observable=1" in FIXTURE,
    ...
    check("export_role=sampleable-presentation" in FIXTURE and "va_status=18 actual_mem_type_supported=1 policy_status=policy-failed" in FIXTURE,
          "sampleable presentation policy status fixture missing")
    check(
    ...
    check("candidate_pixel_source=decoded candidate_pixel_identity_valid=1 candidate_pixel_content_valid=1 candidate_pixel_color_state=zero" in FIXTURE
          and "candidate_pixel_proof_state=zero" in FIXTURE and "candidate_valid=1 reject_reason=none" in FIXTURE,
          "valid zero seed proof fixture missing")
    check("candidate_pixel_source=placeholder" in FIXTURE and "candidate_pixel_content_valid=0" in FIXTURE and "candidate_valid=0 reject_reason=bootstrap-placeholder" in FIXTURE,
          "placeholder seed rejection fixture missing")
    check("event=seed-target-pixel-proof" in FIXTURE and "matches_source=1" in FIXTURE
    ...
    check(
        stream["export_validity_gates"] == 3
    ...
    check("export_validity_gates=3" in text_result.stdout, "text export validity gate aggregate missing")
