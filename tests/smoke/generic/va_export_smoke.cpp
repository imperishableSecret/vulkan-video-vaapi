...
bool validate_nv12_descriptor(const VADRMPRIMESurfaceDescriptor &descriptor, const char *label) {
    if (descriptor.fourcc != VA_FOURCC_NV12 ||
        descriptor.width != smoke_width ||
        descriptor.height != smoke_height ||
        ...
        std::fprintf(stderr,
                     "%s returned unexpected descriptor: fourcc=0x%x size=%ux%u objects=%u layers=%u fd=%d obj_size=%u y=0x%x/%u uv=0x%x/%u\n",
                     label,
                     descriptor.fourcc,
                     descriptor.width,
                     descriptor.height,
                     ...
                     descriptor.num_objects > 0 ? descriptor.objects[0].fd : -1,
                     descriptor.num_objects > 0 ? descriptor.objects[0].size : 0,
                     descriptor.num_layers > 0 ? descriptor.layers[0].drm_format : 0,
                     descriptor.num_layers > 0 ? descriptor.layers[0].pitch[0] : 0,
                     descriptor.num_layers > 1 ? descriptor.layers[1].drm_format : 0,
                     descriptor.num_layers > 1 ? descriptor.layers[1].pitch[0] : 0);
        return false;
    }
    return true;
}

bool export_and_validate(
        VADisplay display,
        VASurfaceID surface,
        VADRMPRIMESurfaceDescriptor *descriptor,
        const char *label) {
    *descriptor = {};
    if (!check_va(vaExportSurfaceHandle(display, surface, VA_SURFACE_ATTRIB_MEM_TYPE_DRM_PRIME_2,
...
int main(void) {
    ...
    if (!check_va(vaCreateConfig(display, VAProfileH264High, VAEntrypointVLD,
    ...
    if (!check_va(vaCreateSurfaces(display, VA_RT_FORMAT_YUV420, smoke_width, smoke_height,
                                   &surface, 1, nullptr, 0),
                  "vaCreateSurfaces[0]")) {
        cleanup();
        return 1;
    ...
    if (!export_and_validate(display, surface, &first_export, "pre-decode export 0") ||
        !export_and_validate(display, surface, &second_export, "pre-decode export 1")) {
    ...
    if (!descriptor_fds_are_open(first_export, "first export") ||
        !descriptor_fds_are_open(second_export, "second export")) {
        close_descriptor_fds(&first_export);
        close_descriptor_fds(&second_export);
        cleanup();
        return 1;
