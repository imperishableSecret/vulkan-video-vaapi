...
int main(void) {
    ...
    bool        ok      = check_va(vaCreateConfig(display, VAProfileH264High, VAEntrypointVLD, &config_attrib, 1, &config), "vaCreateConfig");

    VASurfaceID surface = VA_INVALID_SURFACE;
    if (ok) {
        ok = check_va(vaCreateSurfaces(display, VA_RT_FORMAT_YUV420, smoke_width, smoke_height, &surface, 1, nullptr, 0), "vaCreateSurfaces");
    }

    VAContextID context = VA_INVALID_ID;
    if (ok) {
        ok = check_va(vaCreateContext(display, config, smoke_width, smoke_height, VA_PROGRESSIVE, &surface, 1, &context), "vaCreateContext");
    }

    if (ok) {
        ok                             = expect_status(vaSyncSurface(display, VA_INVALID_SURFACE), VA_STATUS_ERROR_INVALID_SURFACE, "vaSyncSurface(invalid)") && ok;
        VASurfaceStatus surface_status = VASurfaceRendering;
        ok = expect_status(vaQuerySurfaceStatus(display, VA_INVALID_SURFACE, &surface_status), VA_STATUS_ERROR_INVALID_SURFACE, "vaQuerySurfaceStatus(invalid)") && ok;
        ok = check_va(vaQuerySurfaceStatus(display, surface, &surface_status), "vaQuerySurfaceStatus(ready)") && surface_status == VASurfaceReady && ok;
    ...
    if (ok) {
        ...
        ok = check_va(vaQuerySurfaceStatus(display, surface, &surface_status), "vaQuerySurfaceStatus(rendering)") && surface_status == VASurfaceRendering && ok;
        if (surface_status != VASurfaceRendering) {
            std::fprintf(stderr, "active surface status=%d, expected VASurfaceRendering\n", surface_status);
        }
        ok = expect_status(vaSyncSurface2(display, surface, 0), VA_STATUS_ERROR_TIMEDOUT, "vaSyncSurface2(rendering, 0)") && ok;
    }
    ...
    if (ok && surface != VA_INVALID_SURFACE) {
        VASurfaceStatus surface_status = VASurfaceRendering;
        ok = check_va(vaQuerySurfaceStatus(display, surface, &surface_status), "vaQuerySurfaceStatus(after destroy context)") && surface_status == VASurfaceReady && ok;
        if (surface_status != VASurfaceReady) {
