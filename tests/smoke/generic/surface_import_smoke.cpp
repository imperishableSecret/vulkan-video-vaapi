...
bool check_default_import() {
    const VkvvExternalSurfaceImport import =
            vkvv_surface_import_from_attribs(nullptr, 0, 0);
    bool ok = true;
    ok &= check(import.memory_type == VA_SURFACE_ATTRIB_MEM_TYPE_VA,
                "default import should use VA-owned memory");
    ...
    ok &= check(!vkvv_surface_import_has_external_descriptor(nullptr, 0),
                "default import should not report an external descriptor");
    return ok;
}
...
bool check_external_buffers_import(int fd) {
    ...
    set_integer_attrib(&attribs[0], VASurfaceAttribMemoryType,
                       VA_SURFACE_ATTRIB_MEM_TYPE_DRM_PRIME);
    set_pointer_attrib(&attribs[1], VASurfaceAttribExternalBufferDescriptor, &descriptor);

    const VkvvExternalSurfaceImport import =
            vkvv_surface_import_from_attribs(attribs, 2, 0);
    bool ok = true;
    ok &= check(vkvv_surface_import_memory_type(attribs, 2) ==
                        VA_SURFACE_ATTRIB_MEM_TYPE_DRM_PRIME,
                "external buffer import memory type mismatch");
    ok &= check(vkvv_surface_import_has_external_descriptor(attribs, 2),
                "external buffer descriptor not detected");
    ...
    ok &= check(import.width == 1280 && import.height == 720,
                "external buffer import size mismatch");
    ok &= check(fd_identity_matches(import.fd, fd),
                "external buffer fd identity mismatch");
    return ok;
}
...
bool check_drm_prime2_import_index(int fd) {
    ...
    const VkvvExternalSurfaceImport import =
            vkvv_surface_import_from_attribs(attribs, 2, 1);
    bool ok = true;
    ok &= check(import.external, "DRM_PRIME_2 import should be marked external");
    ok &= check(import.memory_type == VA_SURFACE_ATTRIB_MEM_TYPE_DRM_PRIME_2,
                "DRM_PRIME_2 memory type mismatch");
    ok &= check(import.fourcc == fourcc_p010, "DRM_PRIME_2 fourcc mismatch");
    ok &= check(import.width == 3840 && import.height == 2160,
                "DRM_PRIME_2 indexed descriptor size mismatch");
    ok &= check(fd_identity_matches(import.fd, fd),
                "DRM_PRIME_2 fd identity mismatch");
    return ok;
}
...
int main() {
    ...
    ok &= check(vkvv_fd_identity_from_fd(-1).valid == false,
                "negative fd should not produce identity");
