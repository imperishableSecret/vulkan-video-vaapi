...
bool ensure_session(
    ...
    if (!check(video.key.codec_operation == VK_VIDEO_CODEC_OPERATION_DECODE_VP9_BIT_KHR,
...
bool check_vp9_dpb_slots() {
    ...
    if (!check(vkvv::vp9_dpb_slot_for_reference_index(&session, 2) == first_slot,
               "VP9 reference-index lookup did not return the stored slot")) {
        return false;
    }
    if (!check(vkvv::vp9_dpb_slot_for_surface(&session, 41) == first_slot,
               "VP9 surface lookup did not return the stored slot")) {
        return false;
    }
    ...
    return check(vkvv::vp9_dpb_slot_for_reference_index(&session, 2) == second_slot,
                 "VP9 reference slot update did not replace the old slot");
}
...
int main(void) {
    ...
    ok = check(
        (typed_runtime->enabled_decode_operations & VK_VIDEO_CODEC_OPERATION_DECODE_VP9_BIT_KHR) != 0,
        "runtime did not enable VP9 through codec-driven selection") && ok;
    ok = check(typed_runtime->video_decode_vp9,
               "runtime did not enable the VP9 decode feature") && ok;
