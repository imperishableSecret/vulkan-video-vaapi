...
namespace {
    ...
    bool contains_h264_nal_type(const uint8_t* bytes, unsigned int size, unsigned int expected_type) {
    ...
    bool check_idr_access_unit(const VACodedBufferSegment* segment) {
        if (!check_coded_segment(segment, "encoded coded segment is empty")) {
            return false;
        }
        const auto* bytes = static_cast<const uint8_t*>(segment->buf);
