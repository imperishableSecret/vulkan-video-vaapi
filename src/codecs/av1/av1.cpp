...
bool copy_slice_elements(AV1State *av1, const VkvvBuffer *buffer) {
    if (av1 == nullptr || buffer == nullptr || buffer->data == nullptr ||
        buffer->size < sizeof(VASliceParameterBufferAV1) || buffer->num_elements == 0) {
        return false;
    }
...
uint32_t tile_index_from_param(
        const VADecPictureParameterBufferAV1 &pic,
        const VASliceParameterBufferAV1 &param,
        uint32_t fallback) {
...
bool record_tile_offsets(AV1State *av1, int64_t offset_adjustment) {
    ...
    for (const VASliceParameterBufferAV1 &param : av1->pending_slices) {
        ...
        if (adjusted_offset < 0 ||
            static_cast<uint64_t>(adjusted_offset) > std::numeric_limits<uint32_t>::max() ||
...
bool obu_contains_offset(
    ...
    if (obu.data != nullptr && base != nullptr &&
        obu.data >= base && obu.data <= base + buffer_size) {
...
bool parse_av1_frame_header(AV1State *av1, char *reason, size_t reason_size) {
    ...
    while (offset < av1->bitstream.size()) {
        ...
        const GstAV1ParserResult identify_result = gst_av1_parser_identify_one_obu(
            av1->parser,
            av1->bitstream.data() + offset,
            static_cast<uint32_t>(av1->bitstream.size() - offset),
            &obu,
            &consumed);
        if (identify_result != GST_AV1_PARSER_OK || consumed == 0) {
            std::snprintf(reason, reason_size,
                          "failed to identify AV1 OBU: parser_result=%d offset=%u bytes=%zu first=%02x%02x%02x%02x",
                          identify_result,
                          offset,
                          av1->bitstream.size(),
                          av1->bitstream.size() > 0 ? av1->bitstream[0] : 0,
                          av1->bitstream.size() > 1 ? av1->bitstream[1] : 0,
                          av1->bitstream.size() > 2 ? av1->bitstream[2] : 0,
                          av1->bitstream.size() > 3 ? av1->bitstream[3] : 0);
            return false;
        }

        if (obu.obu_type == GST_AV1_OBU_SEQUENCE_HEADER) {
            GstAV1SequenceHeaderOBU sequence{};
            const GstAV1ParserResult parse_result = gst_av1_parser_parse_sequence_header_obu(
                av1->parser, &obu, &sequence);
            if (parse_result != GST_AV1_PARSER_OK) {
                std::snprintf(reason, reason_size,
                              "failed to parse AV1 sequence header: parser_result=%d offset=%u",
                              parse_result, offset);
                return false;
            }
        } else if (obu.obu_type == GST_AV1_OBU_TEMPORAL_DELIMITER) {
            const GstAV1ParserResult parse_result = gst_av1_parser_parse_temporal_delimiter_obu(
                av1->parser, &obu);
            if (parse_result != GST_AV1_PARSER_OK) {
                std::snprintf(reason, reason_size,
                              "failed to parse AV1 temporal delimiter: parser_result=%d offset=%u",
                              parse_result, offset);
                return false;
            }
        } else if (obu.obu_type == GST_AV1_OBU_FRAME_HEADER ||
                   obu.obu_type == GST_AV1_OBU_REDUNDANT_FRAME_HEADER) {
            GstAV1FrameHeaderOBU frame_header{};
            const GstAV1ParserResult parse_result = gst_av1_parser_parse_frame_header_obu(
                av1->parser, &obu, &frame_header);
            if (parse_result != GST_AV1_PARSER_OK) {
                std::snprintf(reason, reason_size,
                              "failed to parse AV1 frame header: parser_result=%d offset=%u",
                              parse_result, offset);
                return false;
            }
            ...
            if (have_target_tile &&
                obu_contains_offset(av1->bitstream.data(), av1->bitstream.size(),
                                    obu, obu_start, consumed, target_tile_offset)) {
            ...
            const GstAV1ParserResult parse_result = gst_av1_parser_parse_frame_obu(
                av1->parser, &obu, &frame);
            if (parse_result != GST_AV1_PARSER_OK) {
                std::snprintf(reason, reason_size,
                              "failed to parse AV1 frame OBU: parser_result=%d offset=%u",
                              parse_result, offset);
                return false;
            }
            have_fallback_header = true;
            ...
            if (have_target_tile &&
                ...
                if (!have_pending_header) {
                    std::snprintf(reason, reason_size,
                                  "AV1 tile group at offset=%u did not have a preceding frame header",
                                  offset);
                    return false;
                }
    ...
    if (selected_updates_reference_state) {
        const GstAV1ParserResult update_result = gst_av1_parser_reference_frame_update(
            av1->parser, &selected_header);
        if (update_result != GST_AV1_PARSER_OK) {
            std::snprintf(reason, reason_size,
                          "failed to update AV1 reference parser state: parser_result=%d offset=%u",
                          update_result, selected_header_offset);
            return false;
        }
    }
...
bool sort_and_validate_tiles(AV1State *av1, char *reason, size_t reason_size) {
    ...
    const uint32_t expected_tile_count =
        static_cast<uint32_t>(av1->pic.tile_cols) * static_cast<uint32_t>(av1->pic.tile_rows);
    if (expected_tile_count == 0 || expected_tile_count > VKVV_AV1_MAX_TILES) {
        std::snprintf(reason, reason_size, "invalid AV1 tile layout: cols=%u rows=%u",
                      av1->pic.tile_cols, av1->pic.tile_rows);
        return false;
    }
    if (av1->tiles.size() != expected_tile_count) {
        std::snprintf(reason, reason_size, "AV1 tile count mismatch: expected=%u got=%zu",
                      expected_tile_count, av1->tiles.size());
        return false;
    }

    std::sort(av1->tiles.begin(), av1->tiles.end(), [](const VkvvAV1Tile &a, const VkvvAV1Tile &b) {
        return a.tile_index < b.tile_index;
    });
    for (uint32_t i = 0; i < expected_tile_count; i++) {
        if (av1->tiles[i].tile_index != i) {
            std::snprintf(reason, reason_size, "AV1 tile index gap: expected=%u got=%u",
                          i, av1->tiles[i].tile_index);
            return false;
        }
    }
    return true;
    ...
    switch (buffer->type) {
        ...
        case VASliceDataBufferType: {
            ...
            if (!av1->pending_slices.empty() &&
                !record_tile_offsets(av1, static_cast<int64_t>(base))) {
                return VA_STATUS_ERROR_INVALID_BUFFER;
            }
            return VA_STATUS_SUCCESS;
        }
...
VAStatus vkvv_av1_prepare_decode(void *state, unsigned int *width, unsigned int *height, char *reason, size_t reason_size) {
    ...
    if (av1->pic.seq_info_fields.fields.mono_chrome ||
        !av1->pic.seq_info_fields.fields.subsampling_x ||
        !av1->pic.seq_info_fields.fields.subsampling_y) {
    ...
    if (av1->pic.frame_width_minus1 == std::numeric_limits<uint16_t>::max() ||
        av1->pic.frame_height_minus1 == std::numeric_limits<uint16_t>::max()) {
    ...
    *height = static_cast<unsigned int>(av1->pic.frame_height_minus1) + 1;
    ...
    std::snprintf(reason, reason_size,
                  ...
                  *width, *height, av1->pic.profile, av1_bit_depth(av1->pic),
                  ...
                  av1->header.frame_header_offset,
                  first_tile_offset,
                  av1->pic.base_qindex);
    return VA_STATUS_SUCCESS;
}
