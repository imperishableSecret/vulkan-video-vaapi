...
namespace {
    ...
    void copy_segmentation_features(const GstVp9SegmentationInfo& segmentation, VkvvVP9FrameHeader* header) {
        ...
        for (uint32_t segment = 0; segment < VKVV_VP9_SEGMENT_COUNT; segment++) {
            ...
            if (src.reference_frame_enabled) {
                ...
                header->segment_feature_data[segment][vp9_seg_level_ref_frame] = static_cast<int16_t>(src.reference_frame);
            }
    ...
    bool copy_gst_header(VP9State* vp9, const GstVp9FrameHdr& src, VkvvVP9FrameHeader* header) {
        ...
        *header                                  = {};
        ...
        header->bit_depth                        = static_cast<uint8_t>(vp9->parser->bit_depth != 0 ? vp9->parser->bit_depth : vp9->pic.bit_depth);
        ...
        header->refresh_frame_flags              = src.frame_type == GST_VP9_KEY_FRAME ? 0xff : static_cast<uint8_t>(src.refresh_frame_flags);
        ...
        std::copy(std::begin(src.loopfilter.ref_deltas), std::end(src.loopfilter.ref_deltas), header->loop_filter_ref_deltas);
        std::copy(std::begin(src.loopfilter.mode_deltas), std::end(src.loopfilter.mode_deltas), header->loop_filter_mode_deltas);
    ...
    bool parse_vp9_frame_header(VP9State* vp9, char* reason, size_t reason_size) {
        if (vp9 == nullptr || vp9->parser == nullptr || !vp9->has_pic || !vp9->has_slice_data || vp9->bitstream.empty()) {
            std::snprintf(reason, reason_size, "missing VP9 parser input");
            return false;
        }

        GstVp9FrameHdr           frame_header{};
        const GstVp9ParserResult result = gst_vp9_parser_parse_frame_header(vp9->parser, &frame_header, vp9->bitstream.data(), vp9->bitstream.size());
        if (result != GST_VP9_PARSER_OK) {
            std::snprintf(reason, reason_size,
                          ...
                          result, vp9->bitstream.size(), vp9->bitstream.size() > 0 ? vp9->bitstream[0] : 0, vp9->bitstream.size() > 1 ? vp9->bitstream[1] : 0,
                          vp9->bitstream.size() > 2 ? vp9->bitstream[2] : 0, vp9->bitstream.size() > 3 ? vp9->bitstream[3] : 0, vp9->pic.pic_fields.bits.frame_type,
                          vp9->pic.pic_fields.bits.show_frame, vp9->pic.frame_header_length_in_bytes, vp9->pic.first_partition_size);
            return false;
        }
    ...
    bool supported_profile_depth(uint8_t profile, uint8_t bit_depth) {
        return (profile == 0 && bit_depth == 8) || (profile == 2 && bit_depth == 10);
    }
...
VAStatus vkvv_vp9_render_buffer(void* state, const VkvvBuffer* buffer) {
    ...
    switch (buffer->type) {
        ...
        case VASliceDataBufferType: {
            ...
            vp9->bitstream.insert(vp9->bitstream.end(), data + slice_offset, data + slice_offset + slice_size);
...
VAStatus vkvv_vp9_prepare_decode(void* state, unsigned int* width, unsigned int* height, char* reason, size_t reason_size) {
    ...
    if (vp9->header.profile != vp9->pic.profile || vp9->header.bit_depth != vp9->pic.bit_depth || vp9->header.subsampling_x != vp9->pic.pic_fields.bits.subsampling_x ||
        ...
        std::snprintf(reason, reason_size, "VP9 VA/header mismatch: va profile=%u depth=%u sub=%u/%u header profile=%u depth=%u sub=%u/%u", vp9->pic.profile, vp9->pic.bit_depth,
                      vp9->pic.pic_fields.bits.subsampling_x, vp9->pic.pic_fields.bits.subsampling_y, vp9->header.profile, vp9->header.bit_depth, vp9->header.subsampling_x,
                      vp9->header.subsampling_y);
        return VA_STATUS_ERROR_INVALID_BUFFER;
    }
    ...
    if (compressed_header_offset > vp9->bitstream.size() || tiles_offset > vp9->bitstream.size()) {
        std::snprintf(reason, reason_size, "VP9 parsed header offsets exceed bitstream: compressed=%u tiles=%u bytes=%zu va_header=%u va_first_partition=%u",
                      compressed_header_offset, tiles_offset, vp9->bitstream.size(), vp9->pic.frame_header_length_in_bytes, vp9->pic.first_partition_size);
        return VA_STATUS_ERROR_INVALID_BUFFER;
    }
