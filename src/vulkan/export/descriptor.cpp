...
VAStatus fill_drm_prime_descriptor(
    ...
    vkvv_trace("export-descriptor-object",
               "surface=%u fourcc=0x%x format=%s fd=%d size=%u modifier=0x%llx has_modifier=%u layers=%u planes=%u visible=%ux%u",
               ...
               plane_count,
               surface != nullptr ? surface->width : 0U,
               surface != nullptr ? surface->height : 0U);
    for (uint32_t i = 0; i < format->layer_count; i++) {
        ...
        vkvv_trace("export-layer-spec",
                   "surface=%u layer=%u drm_format=0x%x aspect=0x%x planes=%u object=%u offset=%u pitch=%u layout_size=%llu row_pitch=%llu array_pitch=%llu depth_pitch=%llu divisor=%ux%u",
                   surface != nullptr ? surface->id : VA_INVALID_ID,
                   i,
                   descriptor->layers[i].drm_format,
                   format->layers[i].aspect,
                   descriptor->layers[i].num_planes,
                   descriptor->layers[i].object_index[0],
                   descriptor->layers[i].offset[0],
                   descriptor->layers[i].pitch[0],
                   static_cast<unsigned long long>(plane_layouts[i].size),
                   static_cast<unsigned long long>(plane_layouts[i].rowPitch),
                   static_cast<unsigned long long>(plane_layouts[i].arrayPitch),
                   static_cast<unsigned long long>(plane_layouts[i].depthPitch),
                   format->layers[i].width_divisor,
