...
namespace vkvv {
    ...
    const ExportFormatInfo* export_format_for_fourcc(unsigned int fourcc) {
        switch (fourcc) {
            case VA_FOURCC_NV12: return &nv12_export_format;
            case VA_FOURCC_P010: return &p010_export_format;
    ...
    const ExportFormatInfo* export_format_for_surface(const VkvvSurface* surface, const SurfaceResource* resource, char* reason, size_t reason_size) {
        ...
        if (resource != nullptr && resource->format != format->vk_format) {
            std::snprintf(reason, reason_size, "surface export format mismatch: va_fourcc=0x%x resource_format=%d expected_format=%d", fourcc, resource->format, format->vk_format);
            return nullptr;
        }
        return format;
    }
