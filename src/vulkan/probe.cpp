...
namespace {
    ...
    struct VideoCapabilitiesChain {
        ...
        explicit VideoCapabilitiesChain(VkStructureType codec_capabilities_type) {
            ...
            switch (codec_capabilities_type) {
                case VK_STRUCTURE_TYPE_VIDEO_DECODE_H264_CAPABILITIES_KHR: codec_caps = &h264; break;
                case VK_STRUCTURE_TYPE_VIDEO_DECODE_H265_CAPABILITIES_KHR: codec_caps = &h265; break;
                case VK_STRUCTURE_TYPE_VIDEO_DECODE_VP9_CAPABILITIES_KHR: codec_caps = &vp9; break;
                case VK_STRUCTURE_TYPE_VIDEO_DECODE_AV1_CAPABILITIES_KHR: codec_caps = &av1; break;
                default: break;
            }
    ...
    bool device_has_surface_export_extensions(VkPhysicalDevice device) {
        ...
        return extension_present(extensions, VK_KHR_EXTERNAL_MEMORY_FD_EXTENSION_NAME) && extension_present(extensions, VK_EXT_EXTERNAL_MEMORY_DMA_BUF_EXTENSION_NAME) &&
    ...
    bool device_supports_export_format(VkPhysicalDevice device, VkFormat format) {
        return device_has_surface_export_extensions(device) && (device_format_has_linear_transfer_dst(device, format) || device_format_has_modifier_transfer_dst(device, format));
    }

    bool device_has_decode_queue(PFN_vkGetPhysicalDeviceQueueFamilyProperties2 get_queue_family_properties2, VkPhysicalDevice device, VkVideoCodecOperationFlagBitsKHR operation) {
        ...
        for (uint32_t i = 0; i < queue_family_count; i++) {
            const bool queue_advertises_decode = (queue_props[i].queueFamilyProperties.queueFlags & VK_QUEUE_VIDEO_DECODE_BIT_KHR) != 0;
    ...
    bool probe_one_profile(bool already_supported, PFN_vkGetPhysicalDeviceQueueFamilyProperties2 get_queue_family_properties2,
                           PFN_vkGetPhysicalDeviceVideoCapabilitiesKHR get_video_capabilities, VkPhysicalDevice device, const VideoProbeProfile& probe,
                           VkvvVideoProfileLimits* limits, char* reason, size_t reason_size) {
    ...
    bool probe_impl(VkvvVideoCaps* caps) {
        ...
        auto           get_queue_family_properties2 =
            reinterpret_cast<PFN_vkGetPhysicalDeviceQueueFamilyProperties2>(vkGetInstanceProcAddr(instance.get(), "vkGetPhysicalDeviceQueueFamilyProperties2"));
        auto get_video_capabilities =
            reinterpret_cast<PFN_vkGetPhysicalDeviceVideoCapabilitiesKHR>(vkGetInstanceProcAddr(instance.get(), "vkGetPhysicalDeviceVideoCapabilitiesKHR"));
        ...
        if (result != VK_SUCCESS || device_count == 0) {
            std::snprintf(caps->summary, sizeof(caps->summary), "no Vulkan physical devices: result=%d count=%u", result, device_count);
            return false;
        }
        ...
        for (VkPhysicalDevice device : devices) {
            ...
            caps->h265_10 = probe_one_profile(caps->h265_10, get_queue_family_properties2, get_video_capabilities, device, probes[2], &caps->h265_10_limits, h265_10_reason,
                                              sizeof(h265_10_reason));
            caps->h265_12 = probe_one_profile(caps->h265_12, get_queue_family_properties2, get_video_capabilities, device, probes[3], &caps->h265_12_limits, h265_12_reason,
                                              sizeof(h265_12_reason));
            caps->vp9    = probe_one_profile(caps->vp9, get_queue_family_properties2, get_video_capabilities, device, probes[4], &caps->vp9_limits, vp9_reason, sizeof(vp9_reason));
            caps->vp9_10 = probe_one_profile(caps->vp9_10, get_queue_family_properties2, get_video_capabilities, device, probes[5], &caps->vp9_10_limits, vp9_10_reason,
                                             sizeof(vp9_10_reason));
            caps->vp9_12 = probe_one_profile(caps->vp9_12, get_queue_family_properties2, get_video_capabilities, device, probes[6], &caps->vp9_12_limits, vp9_12_reason,
                                             sizeof(vp9_12_reason));
