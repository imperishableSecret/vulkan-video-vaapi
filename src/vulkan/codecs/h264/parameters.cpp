...
namespace vkvv {
    ...
    StdVideoH264ProfileIdc std_profile_from_va(VAProfile profile) {
        switch (profile) {
            case VAProfileH264ConstrainedBaseline: return STD_VIDEO_H264_PROFILE_IDC_BASELINE;
            case VAProfileH264Main: return STD_VIDEO_H264_PROFILE_IDC_MAIN;
            case VAProfileH264High:
            default: return STD_VIDEO_H264_PROFILE_IDC_HIGH;
        }
    }

    StdVideoH264ChromaFormatIdc std_chroma_from_va(uint32_t chroma_format_idc) {
        switch (chroma_format_idc) {
            case 0: return STD_VIDEO_H264_CHROMA_FORMAT_IDC_MONOCHROME;
            case 1: return STD_VIDEO_H264_CHROMA_FORMAT_IDC_420;
            case 2: return STD_VIDEO_H264_CHROMA_FORMAT_IDC_422;
            case 3: return STD_VIDEO_H264_CHROMA_FORMAT_IDC_444;
            default: return STD_VIDEO_H264_CHROMA_FORMAT_IDC_INVALID;
        }
    }

    StdVideoH264PocType std_poc_type_from_va(uint32_t pic_order_cnt_type) {
        switch (pic_order_cnt_type) {
            case 0: return STD_VIDEO_H264_POC_TYPE_0;
            case 1: return STD_VIDEO_H264_POC_TYPE_1;
            case 2: return STD_VIDEO_H264_POC_TYPE_2;
            default: return STD_VIDEO_H264_POC_TYPE_INVALID;
        }
    }
    ...
    void build_h264_std_parameters(H264VideoSession* session, VAProfile profile, const VkvvH264DecodeInput* input, H264StdParameters* std_params) {
        ...
        for (uint32_t i = 0; i < STD_VIDEO_H264_SCALING_LIST_4X4_NUM_LISTS; i++) {
            std::fill(std::begin(std_params->sps_scaling.ScalingList4x4[i]), std::end(std_params->sps_scaling.ScalingList4x4[i]), 16);
            std::fill(std::begin(std_params->pps_scaling.ScalingList4x4[i]), std::end(std_params->pps_scaling.ScalingList4x4[i]), 16);
        }
        for (uint32_t i = 0; i < STD_VIDEO_H264_SCALING_LIST_8X8_NUM_LISTS; i++) {
            std::fill(std::begin(std_params->sps_scaling.ScalingList8x8[i]), std::end(std_params->sps_scaling.ScalingList8x8[i]), 16);
            std::fill(std::begin(std_params->pps_scaling.ScalingList8x8[i]), std::end(std_params->pps_scaling.ScalingList8x8[i]), 16);
        }
        if (input->iq != nullptr) {
        ...
        sps.level_idc                                  = derive_h264_level_idc(pic->picture_width_in_mbs_minus1 + 1, pic_height_in_map_units_minus1 + 1, session->max_level);
        ...
        if (input->last_slice_count > 0 && input->last_slices != nullptr) {
            pps.num_ref_idx_l0_default_active_minus1 = static_cast<uint8_t>(std::min<uint32_t>(input->last_slices[0].num_ref_idx_l0_active_minus1, 31));
            pps.num_ref_idx_l1_default_active_minus1 = static_cast<uint8_t>(std::min<uint32_t>(input->last_slices[0].num_ref_idx_l1_active_minus1, 31));
        }
        if (input->all_slices_intra && pic->num_ref_frames >= 3) {
            pps.num_ref_idx_l0_default_active_minus1 = static_cast<uint8_t>(std::min<uint32_t>(pic->num_ref_frames - 2u, 31));
        }
