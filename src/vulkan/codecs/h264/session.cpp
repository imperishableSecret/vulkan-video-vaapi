...
VAStatus vkvv_vulkan_ensure_h264_session(void* runtime_ptr, void* session_ptr, unsigned int width, unsigned int height, char* reason, size_t reason_size) {
    ...
    if (session->video.session != VK_NULL_HANDLE && session->video.key.max_coded_extent.width >= extent.width && session->video.key.max_coded_extent.height >= extent.height) {
        std::snprintf(reason, reason_size, "H.264 video session ready: codec=h264 actual=%ux%u format=%d tiling=%s mem=%llu inline_params=%u",
    ...
    std::snprintf(reason, reason_size,
                  "H.264 video session ready: codec=h264 requested=%ux%u actual=%ux%u format=%d tiling=%s direct_export=%u export_tiling=%u preferred_export_tiling=%u dpb=%u "
                  "refs=%u mem=%llu inline_params=%u",
