...
namespace vkvv {
    ...
    bool reset_h264_session(VulkanRuntime* runtime, H264VideoSession* session, VkVideoSessionParametersKHR parameters, char* reason, size_t reason_size) {
        ...
        if (!record_vk_result(runtime, result, "vkResetFences", "H.264 session reset", reason, reason_size)) {
            return false;
        }
        result = vkResetCommandBuffer(runtime->command_buffer, 0);
        if (!record_vk_result(runtime, result, "vkResetCommandBuffer", "H.264 session reset", reason, reason_size)) {
            return false;
        }
        ...
        if (!record_vk_result(runtime, result, "vkBeginCommandBuffer", "H.264 session reset", reason, reason_size)) {
            return false;
        }
        ...
        if (!record_vk_result(runtime, result, "vkEndCommandBuffer", "H.264 session reset", reason, reason_size)) {
            return false;
        }
    ...
    VkExtent2D h264_session_extent(VkExtent2D requested, const VkVideoCapabilitiesKHR& capabilities) {
        ...
        return {
            std::min(std::max({requested.width, capabilities.minCodedExtent.width, min_nvidia_h264_session_dimension}), capabilities.maxCodedExtent.width),
            std::min(std::max({requested.height, capabilities.minCodedExtent.height, min_nvidia_h264_session_dimension}), capabilities.maxCodedExtent.height),
        };
    }
} // namespace vkvv
...
VAStatus vkvv_vulkan_ensure_h264_session(void* runtime_ptr, void* session_ptr, unsigned int width, unsigned int height, char* reason, size_t reason_size) {
    auto* runtime = static_cast<VulkanRuntime*>(runtime_ptr);
    auto* session = static_cast<H264VideoSession*>(session_ptr);
    ...
    if (session->video.session != VK_NULL_HANDLE && session->video.key.max_coded_extent.width >= extent.width && session->video.key.max_coded_extent.height >= extent.height) {
        std::snprintf(reason, reason_size, "H.264 video session ready: codec=h264 actual=%ux%u format=%d mem=%llu inline_params=%u", session->video.key.max_coded_extent.width,
    ...
    if (extent.width > capabilities.video.maxCodedExtent.width || extent.height > capabilities.video.maxCodedExtent.height) {
        std::snprintf(reason, reason_size, "H.264 coded extent %ux%u exceeds Vulkan limit %ux%u", extent.width, extent.height, capabilities.video.maxCodedExtent.width,
    ...
    if (!choose_decode_format(runtime, &profile_chain.profile, VK_IMAGE_USAGE_VIDEO_DECODE_DST_BIT_KHR | VK_IMAGE_USAGE_VIDEO_DECODE_DPB_BIT_KHR,
                              preferred_vk_format_for_rt_format(VA_RT_FORMAT_YUV420), runtime->surface_export, &format_selection, reason, reason_size)) {
    ...
    if (!record_vk_result(runtime, result, "vkCreateVideoSessionKHR", "H.264 session", reason, reason_size)) {
    ...
    std::snprintf(reason, reason_size, "H.264 video session ready: codec=h264 requested=%ux%u actual=%ux%u format=%d dpb=%u refs=%u mem=%llu inline_params=%u", extent.width,
