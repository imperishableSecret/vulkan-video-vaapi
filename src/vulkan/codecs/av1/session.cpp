...
VAStatus vkvv_vulkan_ensure_av1_session(
        void *runtime_ptr,
        void *session_ptr,
        unsigned int width,
        unsigned int height,
    ...
    if (session->video.session != VK_NULL_HANDLE &&
        ...
        std::snprintf(reason, reason_size,
                      "AV1 video session ready: codec=av1 actual=%ux%u format=%d mem=%llu",
    ...
    if (extent.width > capabilities.video.maxCodedExtent.width ||
        ...
        std::snprintf(reason, reason_size, "AV1 coded extent %ux%u exceeds Vulkan limit %ux%u",
    ...
    std::snprintf(reason, reason_size,
                  "AV1 video session ready: codec=av1 requested=%ux%u actual=%ux%u format=%d dpb=%u refs=%u mem=%llu",
