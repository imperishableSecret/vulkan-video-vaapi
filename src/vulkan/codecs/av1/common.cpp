...
bool valid_reference_slot(const AV1ReferenceSlot &entry) {
    return entry.surface_id != VA_INVALID_ID && entry.slot >= 0 &&
           static_cast<uint32_t>(entry.slot) < max_av1_dpb_slots;
}
...
void av1_set_reference_slot(
    ...
    if (session == nullptr || reference_index >= max_av1_reference_slots ||
        surface_id == VA_INVALID_ID || slot < 0) {
        return;
    }
