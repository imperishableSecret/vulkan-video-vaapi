...
StdVideoVP9InterpolationFilter vp9_interpolation_filter(uint32_t filter) {
    switch (filter) {
        ...
        case STD_VIDEO_VP9_INTERPOLATION_FILTER_SWITCHABLE:
            return static_cast<StdVideoVP9InterpolationFilter>(filter);
        default:
...
VAStatus vkvv_vulkan_decode_vp9(
        void *runtime_ptr,
        void *session_ptr,
        VkvvDriver *drv,
        VkvvContext *vctx,
        VkvvSurface *target,
        VAProfile profile,
        const VkvvVP9DecodeInput *input,
        char *reason,
        size_t reason_size) {
    ...
    if (runtime == nullptr || session == nullptr || drv == nullptr || vctx == nullptr ||
        target == nullptr || input == nullptr || input->pic == nullptr || input->slice == nullptr) {
    ...
    const bool frame_is_inter =
        input->header.frame_type != 0 &&
        input->pic->pic_fields.bits.intra_only == 0;
    ...
    if (frame_is_inter) {
        for (uint32_t i = 0; i < VK_MAX_VIDEO_VP9_REFERENCES_PER_FRAME_KHR; i++) {
            ...
            if (ref_dpb_slot < 0 ||
                session->reference_slots[reference_index].surface_id != ref_surface_id) {
    ...
    if (current_is_reference &&
        (target_dpb_slot < 0 ||
         target_dpb_slot >= static_cast<int>(max_vp9_dpb_slots) ||
    ...
    if (!session->video.initialized &&
        !reset_vp9_session(runtime, session, reason, reason_size)) {
    ...
    std::memcpy(loop_filter.loop_filter_ref_deltas, input->header.loop_filter_ref_deltas,
                sizeof(loop_filter.loop_filter_ref_deltas));
    std::memcpy(loop_filter.loop_filter_mode_deltas, input->header.loop_filter_mode_deltas,
                sizeof(loop_filter.loop_filter_mode_deltas));
    ...
    std::memcpy(segmentation.segmentation_tree_probs, input->pic->mb_segment_tree_probs,
                sizeof(segmentation.segmentation_tree_probs));
    std::memcpy(segmentation.segmentation_pred_prob, input->pic->segment_pred_probs,
                sizeof(segmentation.segmentation_pred_prob));
    std::memcpy(segmentation.FeatureEnabled, input->header.segment_feature_enabled,
                sizeof(segmentation.FeatureEnabled));
    std::memcpy(segmentation.FeatureData, input->header.segment_feature_data,
                sizeof(segmentation.FeatureData));
    ...
    std::memcpy(vp9_picture.referenceNameSlotIndices, reference_name_slot_indices,
                sizeof(vp9_picture.referenceNameSlotIndices));
    ...
    if (result != VK_SUCCESS) {
        ...
        std::snprintf(reason, reason_size,
                      ...
                      vp9_picture.uncompressedHeaderOffset,
                      vp9_picture.compressedHeaderOffset,
                      vp9_picture.tilesOffset,
                      input->bitstream_size);
        return VA_STATUS_ERROR_OPERATION_FAILED;
    }
    ...
    std::snprintf(reason, reason_size,
                  ...
                  reference_count, target_dpb_slot, input->header.refresh_frame_flags,
