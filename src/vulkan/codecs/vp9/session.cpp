...
bool reset_vp9_session(
    ...
    if (result != VK_SUCCESS) {
        std::snprintf(reason, reason_size, "vkResetFences for VP9 session reset failed: %d", result);
        return false;
    }
    result = vkResetCommandBuffer(runtime->command_buffer, 0);
    if (result != VK_SUCCESS) {
        std::snprintf(reason, reason_size, "vkResetCommandBuffer for VP9 session reset failed: %d", result);
        return false;
    }
    ...
    if (result != VK_SUCCESS) {
        std::snprintf(reason, reason_size, "vkBeginCommandBuffer for VP9 session reset failed: %d", result);
        return false;
    }
    ...
    if (result != VK_SUCCESS) {
        std::snprintf(reason, reason_size, "vkEndCommandBuffer for VP9 session reset failed: %d", result);
        return false;
    }
...
VkExtent2D vp9_session_extent(VkExtent2D requested, const VkVideoCapabilitiesKHR &capabilities) {
    return {
        std::min(std::max(requested.width, capabilities.minCodedExtent.width),
                 capabilities.maxCodedExtent.width),
        std::min(std::max(requested.height, capabilities.minCodedExtent.height),
...
VAStatus vkvv_vulkan_ensure_vp9_session(
    ...
    if (session->video.session != VK_NULL_HANDLE &&
        ...
        std::snprintf(reason, reason_size,
                      "VP9 video session ready: codec=vp9 actual=%ux%u format=%d mem=%llu",
    ...
    if (extent.width > capabilities.video.maxCodedExtent.width ||
        ...
        std::snprintf(reason, reason_size, "VP9 coded extent %ux%u exceeds Vulkan limit %ux%u",
    ...
    if (result != VK_SUCCESS) {
        std::snprintf(reason, reason_size, "vkCreateVideoSessionKHR(VP9) failed: %d", result);
    ...
    std::snprintf(reason, reason_size,
                  "VP9 video session ready: codec=vp9 requested=%ux%u actual=%ux%u format=%d dpb=%u refs=%u mem=%llu",
