...
namespace {
    ...
    const char* surface_attrib_name(VASurfaceAttribType type) {
        switch (type) {
            ...
            case VASurfaceAttribExternalBufferDescriptor: return "ExternalBufferDescriptor";
            case VASurfaceAttribUsageHint: return "UsageHint";
            case VASurfaceAttribDRMFormatModifiers: return "DRMFormatModifiers";
            case VASurfaceAttribAlignmentSize: return "AlignmentSize";
            case VASurfaceAttribCount: return "Count";
        }
        return "unknown";
    }
    ...
    const char* generic_value_type_name(VAGenericValueType type) {
        switch (type) {
            case VAGenericValueTypeInteger: return "int";
            case VAGenericValueTypeFloat: return "float";
            case VAGenericValueTypePointer: return "ptr";
            case VAGenericValueTypeFunc: return "func";
        }
        return "unknown";
    }
    ...
    void trace_drm_prime_descriptor(const VADRMPRIMESurfaceDescriptor* descriptor, unsigned int index) {
        if (descriptor == NULL) {
            return;
        }
        ...
        vkvv_trace("surface-import-drm-prime2",
                   ...
                   static_cast<unsigned long long>(fd_stat.dev), static_cast<unsigned long long>(fd_stat.ino), descriptor->num_objects > 0 ? descriptor->objects[0].size : 0,
                   descriptor->num_objects > 0 ? static_cast<unsigned long long>(descriptor->objects[0].drm_format_modifier) : 0ULL,
                   descriptor->num_layers > 0 ? descriptor->layers[0].drm_format : 0, descriptor->num_layers > 0 ? descriptor->layers[0].num_planes : 0,
    ...
    void trace_drm_prime3_descriptor(const VADRMPRIME3SurfaceDescriptor* descriptor, unsigned int index) {
        if (descriptor == NULL) {
            return;
        }
        const int            fd      = descriptor->num_objects > 0 ? descriptor->objects[0].fd : -1;
        ...
        vkvv_trace("surface-import-drm-prime3",
                   ...
                   index, descriptor->fourcc, descriptor->width, descriptor->height, descriptor->num_objects, descriptor->num_layers, fd, fd_stat.valid ? 1U : 0U,
                   static_cast<unsigned long long>(fd_stat.dev), static_cast<unsigned long long>(fd_stat.ino), descriptor->num_objects > 0 ? descriptor->objects[0].size : 0,
                   descriptor->num_objects > 0 ? static_cast<unsigned long long>(descriptor->objects[0].drm_format_modifier) : 0ULL, descriptor->flags,
                   descriptor->num_layers > 0 ? descriptor->layers[0].drm_format : 0, descriptor->num_layers > 0 ? descriptor->layers[0].num_planes : 0,
                   descriptor->num_layers > 0 && descriptor->layers[0].num_planes > 0 ? descriptor->layers[0].object_index[0] : 0,
                   descriptor->num_layers > 0 && descriptor->layers[0].num_planes > 0 ? descriptor->layers[0].offset[0] : 0,
                   descriptor->num_layers > 0 && descriptor->layers[0].num_planes > 0 ? descriptor->layers[0].pitch[0] : 0);
    ...
    void trace_external_buffers_descriptor(const VASurfaceAttribExternalBuffers* descriptor, unsigned int index) {
        if (descriptor == NULL) {
            return;
        }
        ...
        vkvv_trace(
            ...
            static_cast<unsigned long long>(first_buffer), fd_stat.valid ? 1U : 0U, static_cast<unsigned long long>(fd_stat.dev), static_cast<unsigned long long>(fd_stat.ino),
            descriptor->flags, descriptor->num_planes > 0 ? descriptor->pitches[0] : 0, descriptor->num_planes > 0 ? descriptor->offsets[0] : 0,
            descriptor->num_planes > 1 ? descriptor->pitches[1] : 0, descriptor->num_planes > 1 ? descriptor->offsets[1] : 0);
    }

    void trace_create_surface_attribs(uint64_t driver_instance_id, unsigned int format, unsigned int width, unsigned int height, unsigned int num_surfaces,
                                      const VASurfaceAttrib* attrib_list, unsigned int num_attribs) {
        ...
        if (external == NULL || external->value.type != VAGenericValueTypePointer || external->value.value.p == NULL) {
            return;
        }
    ...
    VAStatus complete_vulkan_surface_work(VkvvDriver* drv, VkvvSurface* surface, uint64_t timeout_ns) {
        if (drv == NULL || drv->vulkan == NULL || surface == NULL || !vkvv_surface_has_pending_work(surface)) {
            return sync_surface_work(surface, timeout_ns);
        }

        char     reason[512] = {};
        VAStatus status      = vkvv_vulkan_complete_surface_work(drv->vulkan, surface, timeout_ns, reason, sizeof(reason));
        if (reason[0] != '\0') {
            vkvv_log("%s", reason);
        }
...
VAStatus vkvvCreateSurfaces2(VADriverContextP ctx, unsigned int format, unsigned int width, unsigned int height, VASurfaceID* surfaces, unsigned int num_surfaces,
    ...
    for (unsigned int i = 0; i < num_surfaces; i++) {
        const VkvvExternalSurfaceImport import_info = vkvv_surface_import_from_attribs(attrib_list, num_attribs, i);
        ...
        surface->role_flags         = VKVV_SURFACE_ROLE_DECODE_OUTPUT | (drv->caps.surface_export ? VKVV_SURFACE_ROLE_EXPORTABLE : 0);
...
VAStatus vkvvCreateSurfaces(VADriverContextP ctx, int width, int height, int format, int num_surfaces, VASurfaceID* surfaces) {
    return vkvvCreateSurfaces2(ctx, (unsigned int)format, (unsigned int)width, (unsigned int)height, surfaces, (unsigned int)num_surfaces, NULL, 0);
}
...
VAStatus vkvvQueryImageFormats(VADriverContextP ctx, VAImageFormat* format_list, int* num_formats) {
    VkvvDriver* drv = vkvv_driver_from_ctx(ctx);
    *num_formats    = static_cast<int>(vkvv_query_image_formats(drv, format_list, VKVV_MAX_IMAGE_FORMATS));
    return VA_STATUS_SUCCESS;
}
...
VAStatus vkvvQuerySurfaceAttributes(VADriverContextP ctx, VAConfigID config, VASurfaceAttrib* attrib_list, unsigned int* num_attribs) {
    ...
    set_integer_attrib(&attrib_list[4], VASurfaceAttribPixelFormat, VA_SURFACE_ATTRIB_GETTABLE | VA_SURFACE_ATTRIB_SETTABLE, static_cast<int>(cfg->fourcc));
    set_integer_attrib(&attrib_list[5], VASurfaceAttribMemoryType, VA_SURFACE_ATTRIB_GETTABLE,
                       VA_SURFACE_ATTRIB_MEM_TYPE_VA | (cfg->exportable ? VA_SURFACE_ATTRIB_MEM_TYPE_DRM_PRIME_2 : 0));
