...
namespace {
    ...
    bool decode_context_has_domain(const VkvvContext* vctx) {
        return vctx != NULL && vctx->mode == VKVV_CONTEXT_MODE_DECODE && vctx->stream_id != 0 && vctx->codec_operation != 0 && vctx->width != 0 && vctx->height != 0;
    }
    ...
    void note_decode_domain_locked(VkvvDriver* drv, const VkvvContext* vctx, const VkvvSurface* surface) {
        ...
        if (surface != NULL && surface->driver_instance_id != 0 && drv->driver_instance_id != 0 && surface->driver_instance_id != drv->driver_instance_id) {
            return;
        }
        ...
        const bool         codec_transition = previous_stream != 0 && (previous_stream != vctx->stream_id || previous_codec != vctx->codec_operation);
    ...
    bool surface_matches_active_decode_domain_locked(const VkvvDriver* drv, const VkvvSurface* surface) {
        ...
        if (surface->driver_instance_id != 0 && drv->driver_instance_id != 0 && surface->driver_instance_id != drv->driver_instance_id) {
            return false;
        }
        if (drv->active_decode_width == 0 || drv->active_decode_height == 0 || surface->width != drv->active_decode_width || surface->height != drv->active_decode_height) {
            return false;
        }
