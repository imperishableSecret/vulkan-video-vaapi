...
uint32_t vkvv_surface_import_memory_type(
        const VASurfaceAttrib *attrib_list,
        unsigned int num_attribs) {
...
bool vkvv_surface_import_has_external_descriptor(
        const VASurfaceAttrib *attrib_list,
        unsigned int num_attribs) {
    const VASurfaceAttrib *attrib =
            find_surface_attrib(attrib_list, num_attribs, VASurfaceAttribExternalBufferDescriptor);
    return attrib != nullptr && attrib->value.type == VAGenericValueTypePointer &&
           attrib->value.value.p != nullptr;
}
...
VkvvExternalSurfaceImport vkvv_surface_import_from_attribs(
        const VASurfaceAttrib *attrib_list,
        unsigned int num_attribs,
        unsigned int index) {
    ...
    const VASurfaceAttrib *external =
            find_surface_attrib(attrib_list, num_attribs, VASurfaceAttribExternalBufferDescriptor);
    if (external == nullptr || external->value.type != VAGenericValueTypePointer ||
        external->value.value.p == nullptr) {
        return info;
    }
    ...
    if ((info.memory_type & VA_SURFACE_ATTRIB_MEM_TYPE_DRM_PRIME_3) != 0) {
        ...
        const uintptr_t first_buffer =
                descriptor.buffers != nullptr && descriptor.num_buffers > 0 ? descriptor.buffers[0] : 0;
