...
bool initialize_and_export_second_display(void) {
    ...
    if (!check_va(vaGetConfigAttributes(display, VAProfileH264High, VAEntrypointVLD, &rt_format, 1),
    ...
    if (!check_va(vaCreateConfig(display, VAProfileH264High, VAEntrypointVLD, &config_attrib, 1, &config),
                  "vaCreateConfig")) {
        vaTerminate(display);
        close(fd);
        return false;
